...
class ReferenceMapper {
			...
			if (mf.isMap()) {
				...
				if (refAnn.lazy() && LazyFeatureDependencies.assertDependencyFullFilled()) {
					if (dbObject.containsField(name)) {
						references = mapper.proxyFactory.createListProxy(references, referenceObjClass, refAnn
								.ignoreMissing(), mapper.datastoreProvider);
	...
	void createMapFromDBObject(final BasicDBObject dbObject, final MappedField mf, final Object entity,
			final String name, final Reference refAnn) throws IllegalAccessException {
		Class referenceObjClass = mf.getSubType();
